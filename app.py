...
from helper import most_busy_user, daily_timeline
...
if uploaded_file is not None:
    ...
    if selected_user == 'Overall':
        ...
        with col1:
            ax.bar(x.index,x.values,color = 'indigo')
            st.pyplot(fig)
    ...
    ax.bar(most_common_df[0],most_common_df[1] , color = 'magenta')
    plt.xticks(rotation = 'vertical')
    st.title('Most Commonn Words')
    st.pyplot(fig)
